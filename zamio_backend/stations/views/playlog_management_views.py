    ...
    try:
        ...
        for entry in playlog_entries:
            try:
                ...
                if track:
                    # Import royalty service for calculation
                    from royalties.services import RoyaltyPaymentService
                    
                    # Calculate royalty amount
                    # Note: We'll set this after creating PlayLog since the service expects a PlayLog instance
                    ...
                    # Set royalty amount after creation
                    royalty_amount = RoyaltyPaymentService.calculate_royalty_amount(play_log)
                    play_log.royalty_amount = royalty_amount
                    play_log.royalty_status = 'calculated'
                    play_log.save(update_fields=['royalty_amount', 'royalty_status'])
                    processed_count += 1
                else:
