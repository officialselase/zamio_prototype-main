...
class _StatusPageState extends State<StatusPage>
  ...
  void _onCaptureServiceChanged() {
    if (mounted) {
      setState(() {
        ...
        _backlogCount = _captureService.totalCapturesCreated -
            _captureService.totalCapturesCompleted;
      });
    }
  }
  ...
  void _startService() async {
    if (_stationId.isEmpty || _authToken.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Please login and configure station before starting')));
      return;
    }
    ...
    if (!_isServiceRunning) {
      try {
        ...
        await startForegroundService(
            content: 'Station $_stationId - Offline Mode');
      } catch (e) {
        setState(() {
  ...
  Future<void> _uploadAudioChunk(File file) async {
    ...
    for (var attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          try {
            ...
            if (jsonResponse['match'] == true && mounted) {
              ...
              final songInfo = artistName.isNotEmpty
        ...
        debugPrint(
            'Upload failed (attempt ${attempt + 1}): ${response.statusCode} - $body');
      } catch (e) {
  ...
  Future<void> _scanAndUploadPendingChunks() async {
    try {
      ...
      final activePath =
          _isRecording ? (toggle ? chunkPathA : chunkPathB) : null;
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  // Notice
                  _NoticeBanner(
                      isVisible: !_isServiceRunning,
                      text: 'Service is stopped. Start to capture and upload.'),
                  const SizedBox(height: 12),
                  ...
                  _Glass(
                    child: _HeroStatusCard(
                      ...
                      currentChunkStartedAt:
                          _captureService.currentCaptureStartTime,
                  ...
                  if (_captureService.lastMatchedSong != null ||
                      _captureService.lastDetectionStatus != null)
                    _DetectionResultCard(
                      ...
                      detectionsToday:
                          _captureService.successfulDetectionsToday,
                    ),
                  if (_captureService.lastMatchedSong != null ||
                      _captureService.lastDetectionStatus != null)
                    const SizedBox(height: 16),
                  ...
                  Row(
                    children: [
                      Expanded(
                          child: _StatPill(
                              icon: _connectivity.isConnected
                                  ? Icons.cloud_upload
                                  : Icons.cloud_off,
                              label: 'Status',
                              value: _connectivity.isConnected
                                  ? 'Online'
                                  : 'Offline')),
                      const SizedBox(width: 8),
                      Expanded(
                      ...
                      Expanded(
                          child: _StatPill(
                              ...
                              label: 'Storage',
                              value:
                                  '${_storage.usagePercentage.toStringAsFixed(0)}%')),
                    ],
                  ),
        ...
        return Container(
          decoration: BoxDecoration(
            gradient: LinearGradient(
              ...
              stops: [
                0.0,
                (0.3 + 0.2 * math.sin(t * 2 * math.pi)).clamp(0.2, 0.6),
                1.0
              ],
            ),
          ),
  ...
  void _showStorageDialog(BuildContext context) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
            'Would you like to clean up old captures to free space?'),
        actions: [
          TextButton(
...
class _HeroStatusCard extends StatelessWidget {
  ...
  const _HeroStatusCard(
      ...
      required this.currentChunkStartedAt,
                ...
                children: [
                  ...
                  Positioned(
                    ...
                    child: SizedBox(
                      height: 36,
                      child: _LevelMeter(
                          controller: controller, active: isRecording),
                    ),
                  )
                ],
              ),
...
class _CountdownRing extends StatelessWidget {
  ...
  const _CountdownRing(
      {required this.startedAt, required this.durationSeconds});
...
class _RingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final fg = Paint()
      ..shader =
          const LinearGradient(colors: [Color(0xFF7E57C2), Color(0xFF5E35B1)])
              .createShader(rect)
      ..style = PaintingStyle.stroke
