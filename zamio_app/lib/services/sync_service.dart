...
class SyncService extends ChangeNotifier {
  ...
  Duration _syncInterval =
      const Duration(minutes: 5); // Increased from 2 to 5 minutes
    ...
    try {
      ...
      if (pendingCaptures.isEmpty) {
        ...
        if (_consecutiveEmptySyncs >= _maxEmptySyncsBeforeBackoff) {
          ...
          debugPrint(
              'Queue empty for $_consecutiveEmptySyncs syncs, backing off to $backoffInterval');
  ...
  Future<void> _uploadCapture(AudioCapture capture) async {
    ...
    try {
      ...
      if (success) {
        ...
        // Keep the completed clip locally so operators can play it back from
        // the queue/logs during demos and diagnostics. Storage cleanup owns
        // retention for completed captures.
      } else {
  ...
  Future<bool> _performUpload(AudioCapture capture) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        debugPrint(
            'Upload failed for ${capture.id}: ${response.statusCode} - $responseBody');
        return false;
      }
    } catch (e) {
  ...
  Future<void> _handleUploadFailure(AudioCapture capture,
      {String? error}) async {
