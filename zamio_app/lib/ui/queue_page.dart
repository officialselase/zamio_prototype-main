...
class _QueuePageState extends State<QueuePage> {
                      ...
                      : ListView.separated(
                          ...
                          itemBuilder: (context, i) => RepaintBoundary(
                            child: _CaptureCard(
                              ...
                              capture: captures[i],
                              isPlaying: _playingCaptureId == captures[i].id,
  ...
  Future<void> _deleteCapture(String captureId) async {
    try {
      if (_playingCaptureId == captureId) {
        await _stopPlayback();
      }
  ...
  Future<void> _togglePlayback(AudioCapture capture) async {
    if (!_playerReady) {
      _showSnack('Audio player is still starting');
      return;
    }

    if (_playingCaptureId == capture.id) {
      await _stopPlayback();
      return;
    }
    ...
    if (!await file.exists()) {
      _showSnack('Audio file is no longer available on this device');
      return;
    }

    try {
      if (_playingCaptureId != null) {
        await _stopPlayback();
      }

      await _player.startPlayer(
        fromURI: capture.filePath,
        codec: Codec.aacADTS,
        whenFinished: () {
          if (mounted) {
            setState(() {
              _playingCaptureId = null;
            });
          }
        },
      );
      ...
      if (mounted) {
        setState(() {
          _playingCaptureId = capture.id;
        });
      }
    } catch (e) {
      debugPrint('Playback failed for ${capture.id}: $e');
      _showSnack('Could not play this capture');
      if (mounted) {
        setState(() {
          _playingCaptureId = null;
        });
...
class _CaptureCard extends StatelessWidget {
  ...
  final Function(AudioCapture) onPlayToggle;
        ...
        trailing: Row(
          ...
          children: [
            IconButton(
              ...
              tooltip: isPlaying ? 'Stop playback' : 'Play capture',
              onPressed: () => onPlayToggle(capture),
            ),
